...
export async function generateTrainingPlan(
  ...
  const openai = new OpenAI({
    ...
    defaultHeaders: {
      "HTTP-Referrer": process.env.BASE_URL || "http://localhost:3001",
  ...
  try {
      ...
      messages: [
        {
          role: "system",
          content:
            "You are an expert fitness training and program designer. You must respond with valid JSON only. Do not include any markdown, reasoning, or additional text",
        },
        {
          role: "user",
          content: prompt,
    ...
    console.error("[AI] Error generating AI Gym Plans");
    throw error;
  }
}
...
function buildPrompt(profile: UserProfile): string {
  ...
  return `Create a personalized ${profile.days_per_week}-days per week training plan for someone with the following profile:
